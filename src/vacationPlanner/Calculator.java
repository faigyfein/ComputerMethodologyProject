...
public class Calculator {
    ...
    public static String calculate(ArrayList<Friend> friends) {
       ...
       final double requiredContribution=Math.floor((totalCost/friends.size()) * 100.0) / 100.0; // Amount each friend "owes"
        ...
        double deserves= Math.round(((overpaid.get(friendBeingPaid).getTotalSpent())-requiredContribution) * 100.0) / 100.0;//create and initialize deserves to the amount the friend being payed deserved
        ...
        for(int friend=0;friend<underpaid.size();friend++){//loop through the friends who need to pay
            ...
            while(paid<owes){// While friend in underpaid still owes money
                    if(owes<=deserves){ //If friend in underpaid owes less than friend in overpaid still deserves
                        ...
                        deserves=(overpaid.get(friendBeingPaid).getTotalSpent())-requiredContribution;//set deserves to what next friend in overpaid deserves
                    }
            }
        }
    ...
    private static void splitFriends(ArrayList<Friend> friends, ArrayList<Friend> underpaid, ArrayList<Friend> overpaid, double slice){
